...
import ClassicBuildOrder from "@/components/builds/ClassicBuildOrder.vue";
...
import { getLiveBuild } from "@/composables/data/liveBuildService.js";
...
export default {
  ...
  setup(props) {
    ...
    onMounted(async () => {
      //This view is the production-backed classic reader. It must not take a
      //cached build from the locally configured Firebase project: an identical
      //id can name different data there, and then the page would only sometimes
      //be showing aoe4guides.com. The editor route keeps its existing Firebase
      //read and write path; only this read-only presentation goes to production.
      //
      ...
      if (resBuild) {
        ...
        //Preserve focus-mode deep links even though the classic reader no
        //longer mounts BuildOrderEditor, which used to consume this query.
        if (route.query?.focus) handlePlay("here");
      }
